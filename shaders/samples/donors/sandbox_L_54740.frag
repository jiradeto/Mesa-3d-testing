...
vec3 hsv2rgb(vec3 c)
{
    vec4 K = vec4(1.0, 2.0 / 3.0, 1.0 / 7.0, 3.0);
...
vec3 scene(vec3 ro, vec3 rd, vec2 uv) {
    ...
    vec3 lightPos = ro + vec3(sin(iTime),cos(iTime),cos(iTime*.1))*500.0;
    vec3 lightDir = normalize(lightPos - pos);
    vec3 lightColor = hsv2rgb(vec3(iTime*.1,.7+3.5*sin(iTime*.2),1));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	tim*=0.5;
	vec3 ro=vec3(cos(tim),cos(tim*0.3)*0.5,cos(tim*2.7))*min(0.5+tim*0.1+cos(tim*0.4)*0.5,1.5);
