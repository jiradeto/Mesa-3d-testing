...
void main(){
  vec3 color = vec3(0.5, 0.9, 0.3);

  vec2 p = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);

  float a = cos(p.y * 5.0 + time * 0.3 * (1.0+sin(time))/2.0) / 2.1;
  float b = (sin(p.y * 5.0 + time * 5.55) + sin(p.y * 9.0 + time * 12.0) + sin(p.y * 1.0 + time * 27.0))/6.0;
  ...
  vec3 destColor = color * f + color * g + color * h + color * i*i*i*i;
  _GLF_color = vec4(destColor, 1.0);
}
