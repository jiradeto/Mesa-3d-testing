...
#define COLORFUL 10.0

vec2 tex(vec2 pos){
	return abs(vec2(fract(1379.97721011*sin(pos.x*.372334234)),
		    fract(1739.121885*sin(pos.y*.91))));
}
...
float noise(vec3 x) // iq's 3D noise
{
    ...
    //vec2 d=(uv + 0.5)/256.0, -100.0;
    vec2 rg = tex( (uv + 0.5)/256.0);
...
vec4 scene(vec3 pos, vec3 dir, float t, out float dist)
{
    ...
    d -= rad2 * fbm(pos * mix(1.5, 3.5, sin(t/DURATION*2.*PI)*.5+.5) + time * 0.1);
    dist = d;
    vec4 color;
...
vec3 render(vec3 eye, vec3 dir, vec2 fragCoord, float t)
{
    ...
    vec3 pos = eye + dir * DITHER * tex(gl_FragCoord.xy/resolution.x).r;
...
void main(){
    ...
    if (mouse.y > 0.) angle = mouse.x / resolution.x * 2. * PI;
...
#ifdef COLORFUL
    color = mix(color.rgb, mix(color.bgr, color.brg, sin(time * 0.3)*.5+.5), sin(time * 0.5)*.5+.5);
#endif
