...
vec3 march(vec3 from, vec3 dir) {
  ...
  if (d < det * 2.) {
    ...
    col = mix(col_water * .20, col, fade);
  }
  col *= normalize(col_water + 19.5) * 1.7;
  p = maxdist * dir;
  vec3 bk = fractal(p) * ref * col_water;
  float glow = pow(max(0., dot(dir, -dir_light)), 1.5+eyes_ref*12.8);
  vec3 glow_water = normalize(col_water+1.);
  bk += glow_water*(glow*(1.-eyes_ref*.7) + pow(glow, 20.) * 1.5) * shw * cmouth * ref;
...
void main() {
  ...
  // Set globals
  float atime = mod(time, 600.);
  ani_mouth = sin(atime * 6.);
