...
uniform vec2 injectionSwitch;//Will Jack @SkrillJam
...
vec3 WeirdCircle(vec2 uv, float radius, float thickness, float intensity, vec3 fbmOffset, float t2, vec3 color)
{
   float dist = length(uv);
    ...
    float t1 = intensity *sin(aTan * _Frequency + time *t2*5.0) * 0.5 + 0.5 + fbm(uv,fbmOffset) * 0.15; // weird scaling going on here 
