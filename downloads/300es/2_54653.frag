...
uniform vec2 injectionSwitch;//from
...
void main(){
    ...
    s2=min(1.0,s2*2.2);
    _GLF_color = vec4( mix(vec3(1.0,1.0,0.0),vec3(1.0),pow(s1,60.0))*s1, 1.0 );
    _GLF_color += vec4( mix(mix(vec3(1.0,0.0,0.0),vec3(1.0,0.0,1.0),pow(s2,2.0)),vec3(1.0),pow(s2,10.0))*s2, 1.0 );
	
    _GLF_color.xyz -= vec3(ring(ray,pos,1.03,11.0))*2.0;
    _GLF_color = max( vec4(0.0), _GLF_color );
    
    float s3=ringRayNoise(ray,pos,0.96,1.0,mr,time);
    _GLF_color.xyz += mix(vec3(1.0,0.6,0.1),vec3(1.0,0.95,1.0),pow(s3,3.0))*s3;

    float zero=sphereZero(ray,pos,0.9);
    //if (zero>0.0) {
    	//vec4 s4=noiseSpace(ray,pos,100.0,mr,0.05,vec3(1.0,2.0,4.0),0.0);
    	//s4.x=pow(s4.x,3.0);
    	//_GLF_color.xyz += mix(mix(vec3(1.0,0.0,0.0),vec3(0.0,0.0,1.0),s4.y*1.9),vec3(0.9,1.0,0.1),s4.w*0.75)*s4.x*pow(s4.z*2.5,3.0)*0.2*zero;
    //}

    _GLF_color = max( vec4(0.0), _GLF_color );
    _GLF_color = min( vec4(1.0), _GLF_color );
}