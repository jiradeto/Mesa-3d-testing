...
#define tri(a, b, c) d = min(min(min(d, line(uv, p[a], p[b])), line(uv, p[b], p[c])), line(uv, p[c], p[a]))
...
void main() {
	...
	col += mix(vec3(0), .5 + .5 * cos(time + st.x + 2. * st.y + vec3(0, 1, 2)), sign(cos(st.x)) * sign(cos(st.y))) * t * t;
	...
	col += fract(dot(floor(gl_FragCoord.xy), vec2(0.5, 0.75 * floor(gl_FragCoord.xy).y))) / 8.0;

	
		
	gl_FragColor = vec4(floor(col * 8.0) / 8.0, 1.);
