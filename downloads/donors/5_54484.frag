...
float map(vec3 p) {
	p.x += sin(p.z * 0.1 + time) * 0.1;
	p.y += cos(p.z * 0.1 + time) * 0.1;
	p.xy = rot(p.xy, -p.z * 0.000000000001);
	vec3 m = mod(p, 1.9) - 1.0;
...
void main( void ) {
	...
	vec3 ip = dir * t + pos;
	gl_FragColor = vec4(t * 0.02 * max(vec4(1.0), mix(vec4(1,2,5,4), vec4(5,3,2,1), t * 0.055)));
